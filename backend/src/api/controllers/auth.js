...
const login = async (req, res) => {
    ...
    if (await bcrypt.compare(password, oldUser.password)) {
        ...
        if (res.status(201)) {
            return res.status(200).json({ status: "ok", data: token, userData: oldUser })
        } else {
...
module.exports = { signup, login, registerTeachers, registerStudents, getAllTeachers, getAllStudents }